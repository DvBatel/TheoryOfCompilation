...
namespace ast {
    ...
    Node::Node() : line(yylineno), type_def(""){}
    ...
    std::vector<std::shared_ptr<Exp>> ExpList::getExpList() {
        return this->exps;
    }
    ...
    void Statements::insertFormals(std::vector<std::shared_ptr<Formal>> formals) {
        this->formals = formals;
    }

    std::vector<std::shared_ptr<Formal>> Statements::getFormals()
    {
        return this->formals;
    }
    ...
    std::vector<BuiltInType> Formals::formalTypes() {
        std::vector<BuiltInType> formalTypes;

        for (auto it = this->formals.begin(); it != this->formals.end(); ++it) {
                formalTypes.push_back((*it)->type->getType());
        }

        return formalTypes;
    }

    std::vector<std::shared_ptr<Formal>> Formals::getFormals()
    {
        return this->formals;
    }
    ...
    std::string FuncDecl::getID() {
        return this->id->getVal();
    }

    BuiltInType FuncDecl::getType() {
        return this->return_type->getType();
    }

    std::vector<BuiltInType> FuncDecl::formalTypes() {
        return this->formals->formalTypes();
    }

    std::vector<std::shared_ptr<Formal>> FuncDecl::getFormals()
    {
        return this->formals->getFormals();
    }

    std::shared_ptr<Statements> FuncDecl::getStatememts()
    {
        return this->body;
    }
