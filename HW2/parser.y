...
Funcs: /*epsilon*/												    	{  $$ = std::make_shared<ast::Funcs>(); }
	   | FuncDecl Funcs 												{  (std::dynamic_pointer_cast<ast::Funcs>($2))->push_front(std::dynamic_pointer_cast<ast::FuncDecl>($1));
																		  $$ = $2; }
;
FuncDecl: RetType ID LPAREN Formals RPAREN LBRACE Statements RBRACE {  $$ = std::make_shared<ast::FuncDecl>(std::dynamic_pointer_cast<ast::ID>($2),
...
Exp: LPAREN Exp RPAREN 										        {  $$ = $2; }
   ...
   | Exp AND Exp 													{  $$ = std::make_shared<ast::And>(std::dynamic_pointer_cast<ast::Exp>($1),
                                                                                                      std::dynamic_pointer_cast<ast::Exp>($3)); }
